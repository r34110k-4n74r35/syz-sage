...
from syz_sage.storage import project_root, temporary_directory, writable_path, write_bytes
from syz_sage.sync import UpdateSummary, atomic_write
...
class StoragePathTests(unittest.TestCase):
    ...
    def test_explicit_writes_work_outside_the_default_checkout(self) -> None:
        with mock.patch("syz_sage.storage.project_root", return_value=self.checkout):
    ...
    def test_normalizing_explicit_paths_does_not_need_checkout_or_create_files(self) -> None:
        ...
        with mock.patch("syz_sage.storage.project_root", side_effect=AssertionError("no lookup")):
    ...
    def test_explicit_cli_and_environment_paths_work_without_checkout(self) -> None:
        for selection in ("data-flag", "database-flag", "data-env", "database-env"):
            ...
            with (
                ...
                mock.patch("syz_sage.config.project_root", side_effect=ValueError("no checkout")),
                mock.patch("syz_sage.storage.project_root", side_effect=ValueError("no checkout")),
            ):
    ...
    def test_update_uses_explicit_external_paths(self) -> None:
        ...
        with (
            mock.patch("syz_sage.config.project_root", side_effect=ValueError("no checkout")),
            mock.patch("syz_sage.cli.Updater") as updater,
        ):
    ...
    def test_update_with_only_database_needs_a_default_or_explicit_data_root(self) -> None:
        with (
            mock.patch(
                "syz_sage.config.project_root", side_effect=ValueError("specify --data-dir")
            ),
            mock.patch("syz_sage.cli.Updater") as updater,
        ):
    ...
    def test_inspection_and_migration_need_only_explicit_database(self) -> None:
        ...
        with mock.patch("syz_sage.config.project_root", side_effect=ValueError("no checkout")):
            for arguments in (
                ("status",),
                ("list",),
                ("filter",),
                ...
                with (
                    ...
                    mock.patch("syz_sage.cli.Database", wraps=Database) as factory,
                ):
